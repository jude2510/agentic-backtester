...
def initialize_clients():
    ...
    # Get memory ID. The AgentCore CLI injects MEMORY_QUANT_AGENT_MEM_ID for the
    # in-project memory; fall back to the legacy var, then a name lookup.
    _memory_id = (
        os.getenv('MEMORY_QUANT_AGENT_MEM_ID')
        or os.getenv('QUANT_AGENT_MEMORY_ID')
        or get_memory_id_by_name("quant_agent")
    )
